const { Client, Databases, Query } = require('node-appwrite');

// Initialize Appwrite client
const client = new Client()
  .setEndpoint('https://<YOUR_APPWRITE_DOMAIN>/v1')             // your endpoint
  .setProject('6837256a001912254094')                          // your project ID
  .setKey('standard_4d2b8502ce52c331913fbc1da66da74d684970ff...'); // your function API key

const databases    = new Databases(client);
const DATABASE_ID  = '6842a4150011ed4c7211';      // your buffets DB
const BUFFET_COL   = '6842aa210006eafe1e09';      // your buffs collection
const DELETED_COL  = '687e300e00199e458d7c';      // your deletedBuffets col

module.exports = async function (context) {
  try {
    // 30 minutes ago
    const cutoff = new Date(Date.now() - 30 * 60 * 1000).toISOString();

    // 1) Fetch expired buffets
    const expired = await databases.listDocuments(
      DATABASE_ID,
      BUFFET_COL,
      [ Query.lessThan('clearedby', cutoff) ]
    );

    // 2) Archive & delete
    for (const doc of expired.documents) {
      await databases.createDocument(
        DATABASE_ID,
        DELETED_COL,
        'unique()',
        { ...doc },
        ['*'],
        ['*']
      );
      await databases.deleteDocument(
        DATABASE_ID,
        BUFFET_COL,
        doc.$id
      );
    }

    return context.res.json({
      success: true,
      deletedCount: expired.documents.length
    });
  } catch (err) {
    context.error('BuffetCleaner error:', err);
    return context.res.json({ success: false, message: err.message }, 500);
  }
};
